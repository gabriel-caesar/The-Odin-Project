import { SHIPS_LIST, SHIPS_LIST_TWO } from '../src/Ship';
...
const initializePlayer = (player, name, shipList) => {
  ...
  if (name !== '') {
    // if user inputs a name
    thisPlayer = new Player(name);
  } else if (player == 1 && name === '') {
    // if user doesn't input a name
...
const generateBoardGrid = (board, container) => {
  board.forEach(row => {
    for (let i = 0; i < row.length; i++) {
      ...
      if (container === rightBoard) {
        // differing boards to handle different player's moves
...
const highlightSunkShip = (player, side) => {
  ...
  for (let coordinates of player.gameBoard.sunkShips) {
    const sunkShipSquare = document.getElementById(
      `p${coordinates[0]}-${coordinates[1]}-${side}`
    );
...
const botAttack = (coordinate, player, bot) => {
  ...
  if (map.has([x, y].toString())) {
    // if the missed coordinate was already shot
  ...
  setTimeout(() => {
    ...
    if (!player.gameBoard.receiveAttack(x, y)) {
      ...
      if (shotShip.isSunk()) {
        ...
        if (checkWinner(player, bot, rightBoard)) {
          // auto-explanatory
          return;
      ...
      return botAttack(
        thisSurroundings[zeroTo(thisSurroundings.length)],
        player,
        bot
      );
    }
...
const isFriendGame = () => {
  return !playerBot.checked ? true : false;
};
...
const letTheGamesBegin = () => {
  ...
  if (isFriendGame()) {
    ...
    const innerText = player =>
      `<strong>Friend game:</strong> ${player.name}, position your ships`;
      ...
      if (gameLog.innerHTML === innerText(playerTwo)) {
        ...
        readyButton.addEventListener('click', () => {
          ...
          document
            ...
            .addEventListener('click', () => startDialog.close());
          ...
          shipsInfoButton.addEventListener('click', () => {
            ...
            document
              ...
              .addEventListener('click', () => shipsInfoDialog.close());
          });
    ...
    passButton.addEventListener('click', () => {
      if (gameLog.innerHTML === innerText(playerOne)) {
        // switching turns as pass button's clicked
  ...
  if (!isFriendGame()) {
    ...
    for (let square of rightBoard.children) {
      square.addEventListener('click', () => {
        randomizeButton.style.display = 'none';
        if (
          attackSquares(square.dataset.x, square.dataset.y, computer, 'right')
        ) {
          ...
          if (hit.length !== 0) {
            ...
            return botAttack(
              surroundings[zeroTo(surroundings.length)],
              playerOne,
              computer
            );
          }
